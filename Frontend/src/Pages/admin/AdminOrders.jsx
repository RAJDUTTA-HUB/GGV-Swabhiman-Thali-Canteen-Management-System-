...
const AdminOrders = () => {
  ...
  useEffect(() => {
    const fetchAdminOrders = async () => {
      try {
        const response = await api.get("/admin/admin-orders");
  ...
  const handleClear = async () => {
    try {
      const response = await api.post("/admin/admin-clear-orders");

      alert("All Orders are clear successfully");
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error(error);
    }
  };
  const totalExpense = orders.reduce((sum, day) => sum + day.dayTotal, 0);

  return (
    <div>
      <InsideHeader open={open} setOpen={setOpen} />
      <Sidebar open={open} setOpen={setOpen} role={"admin"} />
      <div className="py-14 px-6">
        ...
        {orders.length !== 0 && (
          <>
            <div className="grid grid-cols-4 gap-4 mb-4 font-mdium text-lg text-gray-800">
              {" "}
        ...
        {orders.length === 0 ? (
          ...
          orders.map((day) => (
            <div key={day.date} className="mb-6">
              {/* Date */}
              <h2 className="font-semibold text-lg border-t-2 border-gray-500 text-gray-800 mb-2">{day.date}</h2>
              ...
              {day.orders.map((item) => (
                ...
                >
                  ...
                  <div className="ml-6">₹{item.amount}</div>
                </div>
              ))}
